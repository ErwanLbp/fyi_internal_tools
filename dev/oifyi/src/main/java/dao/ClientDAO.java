...
public class ClientDAO {
    ...
    public static ArrayList<Client> getAll() throws SQLException {
        ...
        try (Statement req = connection.createStatement()) {
            ...
            while (res.next())
                listeClients.add(new Client(res.getInt("ID_CLIENT"), res.getString("nom"), res.getString("prenom")));
            return listeClients;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static boolean insert(Client client) {
        ...
        try (PreparedStatement req = connection.prepareStatement("INSERT INTO client(ID_CLIENT, NOM, PRENOM) VALUES (?,?,?)")) {
